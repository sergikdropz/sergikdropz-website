async (page) => {
  ...
  const outDir = '/Users/machd/Documents/SERGIK Web and app/web/.tmp/distrokid-wavs'
  ...
  const loggedIn = albumHrefs.length > 0
  if (!loggedIn) {
    return {
      ok: false,
      error: 'Not logged into DistroKid in Playwright session',
      pageUrl: page.url(),
  ...
  for (const [, albumPath] of albumHrefs) {
    ...
    const release = await page.evaluate(() => {
      ...
      const upc =
        clean(
          infoVal(/DistroKid\s*UPC/) ||
            document.querySelector('#js-album-upc')?.textContent ||
            '',
        ).replace(/\D/g, '') || null
      ...
      return {
        albumuuid:
          new URLSearchParams(location.search).get('albumuuid')?.toUpperCase() || '',
        title,
        artist,
    ...
    for (const track of release.tracks) {
      ...
      const safeTitle = String(track.title || 'track')
        ...
        .slice(0, 60)
      const fileName =
        (track.isrc || release.albumuuid + '-' + track.track_number) + '-' + safeTitle + '.wav'
  ...
  return {
    ...
    existed: downloads.filter((d) => d.status === 'exists').length,
