#!/usr/bin/env node
/**
 * Apply release_copyright_checklists.party_contacts column (one-time).
 * Uses POSTGRES_URL_NON_POOLING or POSTGRES_URL from web/.env.local.
 *
 * Usage: node scripts/apply-party-contacts-migration.mjs
 */
...
const sqlFile = path.join(
  webRoot,
  'supabase/migrations/add_party_contacts_to_copyright_checklists.sql',
)

if (!url) {
  console.error('[party-contacts] Set POSTGRES_URL_NON_POOLING in web/.env.local')
  process.exit(1)
}
if (!existsSync(sqlFile)) {
  console.error('[party-contacts] Missing', sqlFile)
  process.exit(1)
}
  ...
  try {
    ...
    const check = await client.query(`
      ...
      WHERE table_schema = 'public'
        ...
        AND column_name = 'party_contacts'
    `)
...
console.log('[party-contacts] Applying release_copyright_checklists.party_contacts…')
try {
  await applyWithPg()
  console.log('[party-contacts] Done.')
} catch (err) {
  console.error('[party-contacts] Failed:', err instanceof Error ? err.message : err)
  console.error(
    '[party-contacts] Or paste supabase/migrations/add_party_contacts_to_copyright_checklists.sql in Supabase SQL Editor',
  )
  process.exit(1)
}
