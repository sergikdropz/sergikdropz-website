...
export default function AdminMusicVault() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [lib, tracks] = await Promise.all([
        ...
        fetchTracks(undefined, { includeArchived: true, includeFullData: false }),
      ])
      setLibraryData(lib)
      setAllTracks(tracks)
  ...
  function navigateUp() {
    ...
    if (parent) {
      ...
      setDisplayFolders(libraryData?.folders || [])
      setDisplayTracks([])
    }
  }
  ...
  return (
    <div className="min-h-screen text-white">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-4">
        ...
        <div className="mb-6">
          <div className="flex flex-col items-center justify-center mb-4 w-full">
            ...
            <p className="text-gray-400 text-xs sm:text-sm mt-1">
              {allTracks.length} songs · Admin View
            </p>
        ...
        {/* Browse Mode */}
        {viewMode === 'browse' && (
          <div className="bg-gray-900/30 border border-gray-800 rounded-xl overflow-hidden" style={{ height: 'calc(100vh - 280px)', minHeight: '500px' }}>
            <ITunesBrowser />
          </div>
        )}
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 right-6 z-50 bg-green-900/90 border border-green-700 text-green-200 px-4 py-2 rounded-lg text-sm shadow-lg">
          {toast}
        </div>
      )}
    </div>
